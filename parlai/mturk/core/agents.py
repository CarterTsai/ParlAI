    ...
    def create_hits(self, opt):
        ...
        for mturk_agent_id in self.mturk_agent_ids:
            for hit_index in range(1, opt['num_hits']+1):
                ...
                mturk_chat_url = self.html_api_endpoint_url + "?method_name=chat_index&task_group_id="+str(self.task_group_id)+"&hit_index="+str(hit_index)+"&all_agent_ids="+all_agent_ids_string+"&cur_agent_id="+str(mturk_agent_id)+"&task_additional_info="+str(opt.get('task_additional_info', ''))
