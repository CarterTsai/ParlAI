...
class MTurkHITAssignmentInfo(Base):
    ...
    id = Column(Integer, primary_key=True)
    task_group_id = Column(String(255), index=True)
    hit_index = Column(String(255), index=True)
...
def is_sane_database(Base, engine):
    ...
    try:
        ...
        test_hit_assignment_info = MTurkHITAssignmentInfo(id=0, task_group_id='Test', hit_index='Test', agent_id='Test')
...
def init_database(host, db_name, username, password):
    ...
    # if not is_sane_database(Base, engine):
...
def get_assignment_index(db_session, task_group_id, hit_index, agent_id):
    new_assignment_object = MTurkHITAssignmentInfo(task_group_id=task_group_id, hit_index=hit_index, agent_id=agent_id)
    ...
    existing_assignment_id_list = db_session.query(MTurkHITAssignmentInfo.id) \
                                    ...
                                    .filter(MTurkHITAssignmentInfo.hit_index==hit_index) \
    ...
    return existing_assignment_id_list.index(object_id) + 1
